...
// A SourceFile is a handle for a file belonging to a FileSet.
// A SourceFile has a name, size, and line offset table.
...
func (f *SourceFile) unpack(offset int, adjusted bool) (filename string, line, column int) {
	f.mutex.Lock()
	filename = f.name
	...
	if adjusted && len(f.infos) > 0 {
		...
		if i := searchLineInfos(f.infos, offset); i >= 0 {
			...
			if i := searchInts(f.lines, alt.Offset); i >= 0 {
				// i+1 is the line at which the alternative positixn was recorded
...
func (f *SourceFile) position(p Pos, adjusted bool) (pos Position) {
	offset := int(p) - f.base
	pos.Offset = offset
...
type FileSet struct {
	...
	last  atomic.Pointer[SourceFile] // cache of last file looked up
}
...
func (s *FileSet) AddFile(filename string, base, size int) *SourceFile {
	...
	f := &SourceFile{name: filename, size: size, lines: []int{0}}
